...
# 模块信息
module_name = "RobotMsgConfig"  # 模块名称
module_model = RobotMsgConfig  # 对应的模型类
...
def queryByPage():
    ...
    try:
        # 分页查询
        coll_id = request.json.get("coll_id","") # 获取数据集的id
        coll_type = request.json.get("coll_type","") # 获取数据集的coll_type
        # coll_type = request.json["coll_type"] # 获取当前测试集合的类型
        # page = int(request.json["page"])
        # page_size = int(request.json["pageSize"])
        with application.app_context():
            filter_list = []
            if coll_id:
                filter_list.append(module_model.coll_id == int(coll_id))
            # ====筛选条件(如果有筛选条件，在这里拓展 - filter)
            # 代码示例：
            # yyyy = request.json["xxxx"]
            if len(coll_type) > 0 :
               filter_list.append(module_model.coll_type.like(f"%{coll_type}%"))
            # =====结束
            # 数据库查询
...
def update():
    ...
    try:
        with application.app_context():
            ...
            elif "coll_id" in request.json:
                # 按 coll_id 更新
                module_model.query.filter_by(coll_id=request.json["coll_id"]).update(request.json)
            else:
                return respModel.error_resp(msg="请求参数错误: 缺少 id 或 coll_id")
            database.session.commit()
