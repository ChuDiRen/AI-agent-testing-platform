...
class KnowledgeBase(Base):
    ...
    user_id = Column(Integer, ForeignKey("user.id"), nullable=False, comment="创建用户ID")
...
class Document(Base):
    ...
    user_id = Column(Integer, ForeignKey("user.id"), nullable=False, comment="上传用户ID")
...
class DocumentChunk(Base):
    ...
    meta = Column("metadata", Text, comment="元数据(JSON格式)")
...
class KnowledgeQuery(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    knowledge_base_id = Column(String(36), ForeignKey("knowledge_bases.id"), nullable=False, comment="知识库ID")
    user_id = Column(Integer, ForeignKey("user.id"), nullable=False, comment="查询用户ID")
