...
const TestRunReportDetail: React.FC = () => {
  ...
  const renderScorecard = () => (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <Card className="bg-slate-800/50 border-slate-700/50 backdrop-blur-sm">
        ...
        />
        <Progress percent={Math.round((MOCK_SUMMARY.passed / MOCK_SUMMARY.total) * 100)} showInfo={false} strokeColor="#4ade80" trailColor="rgba(255,255,255,0.1)" size="small" className="mt-2" />
      </Card>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 backdrop-blur-sm">
        <Statistic 
          ...
          valueStyle={{ color: '#cbd5e1', fontSize: '1.25rem' }}
  ...
  const renderFailedCases = () => (
    ...
    >
      {MOCK_FAILED_CASES.map(testCase => (
        <Panel 
          key={testCase.id} 
          header={
        ...
        >
          <div className="flex flex-col gap-4 p-2">
            {/* Error Message */}
            <Alert
              message={<span className="font-mono font-bold text-red-400">Error</span>}
            ...
            {/* AI Analysis */}
            {testCase.aiAnalysis && (
              <div className="bg-indigo-950/30 border border-indigo-500/30 rounded-lg p-4 relative overflow-hidden">
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
           ...
           <Button type="primary" className="bg-indigo-600 hover:bg-indigo-500">Rerun Failed</Button>
        </div>
      </div>
          ...
          },
          {
            ...
            children: (
              <div className="grid grid-cols-4 gap-4">
                 {[1, 2, 3].map(i => (
                   <Card key={i} className="bg-slate-800/50 border-slate-700/50 hover:border-indigo-500/50 cursor-pointer group transition-all">
