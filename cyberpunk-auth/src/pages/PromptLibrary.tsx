    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">Manage and reuse your AI instruction templates</p>
        </div>
        <div className="flex items-center gap-3">
           <Button type="primary" icon={<Sparkles size={16} />} className="!bg-indigo-600 hover:!bg-indigo-500">
             Create Template
           </Button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 p-1">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-slate-900 border border-slate-800 rounded-lg pl-10 pr-4 py-2 text-sm text-slate-200 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all"
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTemplates.map(template => (
          <div key={template.id} className="group bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-xl p-5 hover:border-indigo-500/50 transition-all hover:shadow-[0_0_20px_rgba(99,102,241,0.1)] flex flex-col h-full">
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-2 text-xs font-medium px-2 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-300">
            ...
            <div className="bg-slate-950 rounded-lg p-3 mb-4 border border-slate-800 flex-1 relative overflow-hidden group/code">
              <code className="text-xs text-slate-300 font-mono block line-clamp-4">
                {template.content}
              </code>
            ...
            <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-800/50">
              <div className="flex gap-2">
                {template.tags.map(tag => (
                  <span key={tag} className="text-[10px] text-slate-500 bg-slate-800/50 px-1.5 py-0.5 rounded border border-slate-800">
                    #{tag}
                  </span>
                ))}
              </div>
              <Button 
                size="small" 
                className="!bg-indigo-600/10 !text-indigo-400 !border-indigo-500/20 hover:!bg-indigo-600 hover:!text-white hover:!border-indigo-600 transition-all flex items-center gap-1.5"
              >
                <Sparkles size={14} />
                Use Template
              </Button>
