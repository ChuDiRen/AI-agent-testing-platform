...
class RoleManagementPage:
    ...
    def __init__(self, page):
        ...
        self.search_input = page.locator('input[placeholder*="角色名"], input[placeholder*="搜索"]')
    ...
    def goto(self):
        self.page.goto(f"{WEB_BASE_URL}/system/role")
    ...
    def click_row_permission(self, index: int = 0):
        self.page.locator(f'.el-table__row:nth-child({index+1}) button:has-text("权限")').click()


class TestRolePageDisplay:
    """角色管理页面展示测试"""
    ...
    def test_page_display(self):
        """角色管理页面正常加载"""
        page_obj = RoleManagementPage(self.page)
        page_obj.goto()
