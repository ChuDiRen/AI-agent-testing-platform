...
class Text2CaseState(TypedDict):
    """Text2Case状态定义"""
    messages: List[dict]
    requirement: str
    test_type: str
...
def extract_requirement(state: Text2CaseState) -> str:
    """从状态中提取需求"""
...
def analyze_requirement(state: Text2CaseState) -> Text2CaseState:
    """分析需求"""
    logger.info("Analyzing requirement...")
    
    model = get_model()
    requirement = extract_requirement(state)
    ...
    try:
        ...
        return {
            **state,
            ...
            "analysis": analysis,
            "messages": state.get("messages", []) + [{"role": "ai", "content": f"需求分析完成:\n{analysis}"}]
        }
...
def design_test_points(state: Text2CaseState) -> Text2CaseState:
    """设计测试点"""
    ...
    try:
        ...
        return {
            **state,
            ...
            "messages": state.get("messages", []) + [{"role": "ai", "content": f"测试点设计完成，共{len(test_points)}个测试点"}]
        }
    except Exception as e:
...
def write_test_cases(state: Text2CaseState) -> Text2CaseState:
    """编写测试用例"""
    ...
    try:
        ...
        return {
            **state,
            ...
            "iteration": iteration,
            "messages": state.get("messages", []) + [{"role": "ai", "content": f"测试用例编写完成，共{len(test_cases)}个用例"}]
        }
    except Exception as e:
...
def review_test_cases(state: Text2CaseState) -> Text2CaseState:
    """评审测试用例"""
    ...
    try:
        ...
        return {
            **state,
            ...
            "completed": completed,
            "messages": state.get("messages", []) + [{"role": "ai", "content": f"评审完成，总分: {total_score}"}]
        }
    except Exception as e:
...
def should_continue(state: Text2CaseState) -> str:
    """决定是否继续迭代"""
    if state.get("error"):
        return END
...
def build_graph():
    """构建LangGraph图"""
    workflow = StateGraph(Text2CaseState)
