...
class AiMessage(SQLModel, table=True):
    ...
    content: str = Field(sa_column_kwargs={"type_": "LONGTEXT"})  # 消息内容
    message_type: str = Field(max_length=20, default="text", index=True)  # 消息类型（text/testcase/error/file/command）
    test_cases_json: Optional[str] = Field(default=None, sa_column_kwargs={"type_": "LONGTEXT"})  # 生成的测试用例JSON
    metadata: Optional[str] = Field(default=None, sa_column_kwargs={"type_": "TEXT"})  # 元数据（如文件信息、参数等）
