...
# ==================== 用例查询相关 ====================
class ApiCaseInfoQuery(BaseModel):
    """API用例查询请求"""
...
# ==================== 步骤相关 ====================
class ApiInfoStepCreate(BaseModel):
    """API用例步骤创建请求"""
...
class ApiInfoStepUpdate(BaseModel):
    """API用例步骤更新请求"""
...
class ApiInfoStepResponse(BaseModel):
    """API用例步骤响应"""
    id: int
    case_info_id: int
    run_order: int
...
# ==================== 用例创建/更新相关 ====================
class ApiCaseInfoCreate(BaseModel):
    """API用例创建请求"""
    project_id: Optional[int] = Field(default=None, description="项目ID")
    case_name: str = Field(description="用例名称")
    case_desc: Optional[str] = Field(default=None, description="用例描述")
    steps: Optional[List[ApiInfoStepCreate]] = Field(default=None, description="用例步骤列表")

class ApiCaseInfoUpdate(BaseModel):
    """API用例更新请求"""
...
# ==================== 用例响应相关 ====================
class ApiCaseInfoResponse(BaseModel):
    """API用例响应（基本信息）"""
    id: int
...
class ApiCaseWithSteps(BaseModel):
    """API用例详情响应（含步骤）"""
    id: int
    project_id: Optional[int]
    case_name: str
    ...
    create_time: Optional[str]
    modify_time: Optional[str]
    steps: List[ApiInfoStepResponse] = Field(default=[], description="用例步骤列表")
...
# ==================== 用例执行相关 ====================
class ApiCaseExecuteRequest(BaseModel):
    """API用例执行请求"""
    case_id: int = Field(description="用例ID")
    test_name: Optional[str] = Field(default=None, description="测试名称")
    context_vars: Optional[Dict[str, Any]] = Field(default=None, description="上下文变量")

class ApiCaseExecuteResponse(BaseModel):
    """API用例执行响应"""
