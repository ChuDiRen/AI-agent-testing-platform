...
class KeywordBase(BaseModel):
    ...
    parameters: Optional[List[KeywordParamSchema]] = Field(None, description="参数定义")
...
class KeywordSimpleResponse(BaseModel):
    """关键字简化响应Schema(用于内置关键字)"""
    name: str = Field(..., description="关键字名称")
    description: Optional[str] = Field(None, description="关键字描述")
    parameters: Optional[List[Dict[str, Any]]] = Field(None, description="参数定义")
    is_builtin: bool = Field(True, description="是否内置关键字")


class KeywordListResponse(BaseModel):
    """关键字列表响应Schema"""
    total: int = Field(..., description="总数")
    items: List[KeywordSimpleResponse] = Field(..., description="关键字列表")  # 改用简化Schema
