  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        >
          <h1 className="text-3xl font-bold font-display text-white mb-1 drop-shadow-md flex items-center gap-3">
            <Activity className="text-neon-cyan" />
            Quality Analytics
          </h1>
          <p className="text-slate-400 font-mono text-sm">Deep dive into test stability, performance, and coverage metrics.</p>
        </motion.div>
        <motion.div
          initial={{ x: 20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
        ...
        >
            <Tag className="bg-slate-900 border-slate-700 text-slate-300 px-3 py-1 text-xs font-mono">Last 7 Days</Tag>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Pass Rate Trend */}
        <CyberCard delay={0.2} title={<span className="text-slate-200 flex items-center gap-2 font-display"><TrendingUp size={16} className="text-neon-green" /> Pass Rate Trend</span>}>
          <div className="h-[300px] w-full pt-4" style={{ minHeight: '300px' }}>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={TREND_DATA} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <XAxis dataKey="date" stroke="#64748b" tick={{ fontSize: 12, fontFamily: 'monospace' }} />
                <YAxis stroke="#64748b" domain={[80, 100]} tick={{ fontSize: 12, fontFamily: 'monospace' }} />
        ...
        {/* Duration Trend */}
        <CyberCard delay={0.3} title={<span className="text-slate-200 flex items-center gap-2 font-display"><BarChart3 size={16} className="text-neon-purple" /> Avg. Duration Trend</span>}>
          <div className="h-[300px] w-full pt-4" style={{ minHeight: '300px' }}>
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart data={TREND_DATA} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                <XAxis dataKey="date" stroke="#64748b" tick={{ fontSize: 12, fontFamily: 'monospace' }} />
                <YAxis stroke="#64748b" tick={{ fontSize: 12, fontFamily: 'monospace' }} unit="s" />
                <RechartsTooltip content={<CustomTooltip />} />
                <Bar 
                  dataKey="duration" 
                  name="Duration" 
                  ...
                  fillOpacity={0.8}
                />
                <Line type="monotone" dataKey="total" name="Total Tests" stroke="#94a3b8" dot={false} strokeDasharray="5 5" strokeWidth={2} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </CyberCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
            <CyberCard delay={0.4} className="h-full" title={<span className="text-slate-200 flex items-center gap-2 font-display"><AlertTriangle size={16} className="text-yellow-400" /> Top Flaky Tests</span>}>
                <Table 
                    ...
                    columns={[
                        ...
                        { 
                            ...
                            dataIndex: 'module', 
                            key: 'module',
                            render: (text: string) => <Tag className="bg-slate-950 border-slate-800 text-slate-400 font-mono">{text}</Tag>
                        },
                        { 
                            title: 'Failure Rate', 
                            ...
                            render: (rate: number) => (
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden w-24">
                                        <div className="h-full bg-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.5)]" style={{ width: `${rate}%` }}></div>
                                    </div>
                                    <span className="text-yellow-500 font-bold text-xs font-mono">{rate}%</span>
                                </div>
                            )
                        },
                        { 
                            ...
                            dataIndex: 'lastFail', 
                            key: 'lastFail',
                            render: (text: string) => <span className="text-slate-500 text-xs font-mono">{text}</span>
                        }
                    ]}
                />
            </CyberCard>
        ...
        <div className="lg:col-span-1">
            <CyberCard delay={0.5} className="h-full" title={<span className="text-slate-200 flex items-center gap-2 font-display"><Shield size={16} className="text-neon-cyan" /> API Coverage Map</span>}>
                    ...
                    {API_COVERAGE_MODULES.map((mod, i) => (
                        <Tooltip key={mod.name} title={`${mod.endpoints} Endpoints - ${mod.coverage}% Covered`}>
                            <motion.div 
                                ...
                                className={`
                                p-3 rounded-lg border backdrop-blur-sm transition-all hover:scale-[1.05] cursor-default
                            ...
                            `}>
                                <div className="flex justify-between items-start">
                                    ...
                                    {mod.critical && <Activity size={12} className="opacity-70 animate-pulse" />}
                                </div>
                                <div>
                                    <div className="text-2xl font-bold font-display">{mod.coverage}%</div>
                                    <div className="text-[10px] opacity-70 font-mono">{mod.endpoints} EPS</div>
                                </div>
                            </motion.div>
                        </Tooltip>
                    ))}
                ...
                <div className="mt-4 flex gap-4 justify-center text-xs text-slate-500 font-mono">
                    <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)]"></div> High</span>
                    <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_5px_rgba(59,130,246,0.5)]"></div> Good</span>
                    <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-yellow-500 shadow-[0_0_5px_rgba(234,179,8,0.5)]"></div> Med</span>
                    <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_5px_rgba(239,68,68,0.5)]"></div> Low</span>
                </div>
            </CyberCard>
        </div>
      </div>
