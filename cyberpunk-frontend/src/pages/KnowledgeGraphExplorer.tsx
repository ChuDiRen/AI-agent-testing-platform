...
const NodeIcon = ({ type, size = 20 }: { type: string; size?: number }) => {
  switch (type) {
    case 'service': return <Server size={size} className="text-neon-cyan" />;
    ...
    <div className="relative w-full h-[calc(100vh-64px)] overflow-hidden bg-[#0a0a0f] text-slate-200">
      ...
      <div className="absolute top-6 left-6 w-80">
        <Input 
          ...
          className="bg-slate-900/80 backdrop-blur-md border-slate-700/50 text-slate-200 focus:border-neon-cyan hover:border-slate-600 h-10 rounded-lg"
          allowClear
        />
      </div>
      ...
      <div className="absolute top-6 right-6 bg-slate-900/80 backdrop-blur-md p-4 rounded-xl border border-slate-700/50 shadow-lg pointer-events-none">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-slate-300">
            <span className="w-2 h-2 rounded-full bg-neon-cyan"></span> Service
          </div>
          <div className="flex items-center gap-2 text-xs text-slate-300">
        ...
        {selectedNode && (
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {selectedNode.documents.length > 0 ? selectedNode.documents.map((doc, idx) => (
                  <div key={idx} className="bg-slate-800/30 p-3 rounded border border-slate-700/50 hover:border-neon-cyan/50 transition-colors cursor-pointer group">
            ...
            <div className="pt-4 border-t border-slate-800">
              <Space>
                <Button type="primary" className="bg-neon-cyan hover:bg-neon-cyan">Edit Node</Button>
