...
from apitest.model.ApiOperationTypeModel import OperationType  # 请替换为你的ApiModule模型类
...
def queryByPage():
    ...
    try:
        ...
        with application.app_context():
            filter_list = []
            # 添加操作类型模糊搜索条件
            operation_type_name = request.json.get("operation_type_name", "")
            if len(operation_type_name) > 0:
                filter_list.append(module_model.operation_type_name.like(f'%{operation_type_name}%'))
            # 数据库查询
