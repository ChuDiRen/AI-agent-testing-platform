    ...
    <div v-if="currentView === 'list'">
      ...
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-row class="mb-4" type="flex" justify="end">
          ...
          <el-button type="warning" @click="onDataForm(-1)">新增部门</el-button>
        </el-row>
      </el-form>
...
const loadData = () => {
  ...
  deptApi.queryByPage(searchData).then((res) => {
...
const onDelete = async (index) => {
  const deptId = tableData.value[index]["id"]
  const deptName = tableData.value[index]["name"]

  await confirmDelete(
    () => deptApi.deleteData(deptId),
    `确定要删除部门 "${deptName}" 吗？此操作不可恢复！`,
    '部门删除成功',
    loadData
  )
}
