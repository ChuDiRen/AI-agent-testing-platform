      ...
      <el-table :data="tableData" style="width: 100%;" max-height="500">
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #default="scope">
            ...
            <el-button link type="primary" size="small" @click.prevent="onMenuPermission(scope.$index)">
              菜单权限
            </el-button>
            <el-button link type="primary" size="small" @click.prevent="onApiPermission(scope.$index)">
              API权限
            </el-button>
...
const onMenuPermission = (index) => {
  const roleId = tableData.value[index]["id"]
  router.push(`/system/roles/roleMenu/${roleId}`)
}

const onApiPermission = (index) => {
  const roleId = tableData.value[index]["id"]
  router.push(`/system/roles/roleApi/${roleId}`)
}
