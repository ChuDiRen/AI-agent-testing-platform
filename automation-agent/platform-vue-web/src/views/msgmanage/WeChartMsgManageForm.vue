...
import { messageConfig, type FormInstance, type FormRules } from 'element-plus';
...
const ruleForm = reactive({
  id: 0,
  robot_name: '',
  robot_type: 1,  // 钉钉机器人类型，1：企业微信，2：钉钉人，3：飞书
  webhook_url: '',
  keywords: '',
  message_template: ''
});
    ...
    if (ruleForm.id > 0) {
      updateData(ruleForm).then((res: { data: { code: number; msg: string; }; }) => {
        if (res.data.code == 200) {
          router.push('/WeChartMsgManageList'); // 跳转回列表页面
        }
      });
    } else {
...
// 6. 关闭表单 - 回到数据列表页
const closeForm = () => {
  router.push('/WeChartMsgManageList');
};
