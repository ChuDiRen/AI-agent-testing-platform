...
**设计风格自动推断规则：**
- 用户已指定风格 → 使用用户指定风格
- 用户未指定，项目类型为"管理后台" → 使用 SaaS 蓝 + Minimalism 风格
- 用户未指定，项目类型为"电商前台" → 使用 活力橙 + Flat Design 风格
- 用户未指定，其他类型 → 使用 科技紫 + Minimalism 风格

**关键原则：**
- ✅ 优先使用用户在 prompt 中明确指定的风格
- ✅ 未指定时根据项目类型自动推断，不询问用户
- ✅ 确保设计风格的一致性和专业度
