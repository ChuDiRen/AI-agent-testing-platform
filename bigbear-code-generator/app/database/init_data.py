...
def load_static_menu_config():
    ...
    try:
        ...
        project_root = current_dir.parent.parent  # bigbear-code-generator 目录
        menu_config_file = project_root / "web" / "src" / "config" / "staticMenus.js"
        ...
        temp_script = project_root / "app" / "temp_parse_menu.js"
    ...
    try:
        with Session(engine) as session:
            ...
            for menu_id in menu_ids:
                ...
                if not existing:
                    # 为所有菜单分配权限(包括目录、菜单和按钮)
                    menu = session.get(Menu, menu_id)
                    if menu:
                        role_menu = RoleMenu(role_id=1, menu_id=menu_id)
                        session.add(role_menu)
                        if menu.perms:
                            logger.info(f"为超级管理员分配菜单: {menu.menu_name} (权限: {menu.perms})")
                        else:
                            logger.info(f"为超级管理员分配菜单: {menu.menu_name} (目录)")
